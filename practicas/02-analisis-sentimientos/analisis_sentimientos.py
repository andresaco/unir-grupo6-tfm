...
def print_banner():
    ...
    print(
        f"{Colors.BOLD}Modelo recomendado:{Colors.ENDC} lxyuan/distilbert-base-multilingual-cased-sentiments-student"
    )
    print(f"{Colors.BOLD}Hardware Detectado:{Colors.ENDC}")
    print(
        f"  - CPU Cores: {psutil.cpu_count(logical=True)} (Lógicos) / {psutil.cpu_count(logical=False)} (Físicos)"
    )
    ram = psutil.virtual_memory()
    print(
        f"  - Memoria RAM Total: {ram.total / (1024**3):.2f} GB (Disponible: {ram.available / (1024**3):.2f} GB)"
    )
    print(
        f"  - Dispositivo de cálculo: {'GPU (CUDA)' if torch.cuda.is_available() else 'CPU (Recomendado para tu setup)'}"
    )
...
def validate_inputs(file_path, column_name):
    ...
    if column_name not in df_head.columns:
        ...
        raise KeyError(
            f"La columna '{column_name}' no existe en el CSV. Columnas disponibles: {cols_available}"
        )
...
def load_sentiment_pipeline():
    ...
    print(
        f"{Colors.WARNING}Cargando el modelo '{model_name}' en memoria...{Colors.ENDC}"
    )
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}✔ Modelo cargado con éxito en {elapsed:.2f} segundos.{Colors.ENDC}\n"
        )
        return nlp_pipeline
    except Exception as e:
...
def run_sentiment_analysis(file_path, column_name, batch_size=32):
    ...
    with tqdm(
        total=total_rows, desc="Procesando", unit="filas", colour="green"
    ) as pbar:
        for i in range(0, total_rows, batch_size):
            ...
            try:
                ...
                # Manejo de errores: si un lote falla, procesar fila por fila
                print(
                    f"{Colors.FAIL}Error procesando lote {i}-{i + batch_size}: {e}{Colors.ENDC}"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Script optimizado de análisis de sentimientos local para CPU."
    )
    parser.add_argument("--file", type=str, help="Ruta del archivo CSV de entrada.")
    parser.add_argument(
        "--col", type=str, help="Nombre de la columna de texto a analizar."
    )
    parser.add_argument(
        "--batch",
        type=int,
        default=16,
        help="Tamaño de lote (batch size). Por defecto es 16 (ideal para CPU).",
    )
    ...
    if not args.file or not args.col:
        print_banner()
        print(
            f"{Colors.WARNING}No se detectaron argumentos de consola. Entrando en modo interactivo...{Colors.ENDC}\n"
        )
        ...
        if not os.path.exists(file_input):
            print(
                f"{Colors.FAIL}El archivo indicado no existe. Saliendo...{Colors.ENDC}"
            )
            sys.exit(1)
        ...
        col_input = input(
            "2. Escribe el nombre exacto de la columna a analizar: "
        ).strip()
        batch_input_str = input(
            "3. Tamaño de lote (Presiona Enter para usar 16 por defecto): "
        ).strip()
