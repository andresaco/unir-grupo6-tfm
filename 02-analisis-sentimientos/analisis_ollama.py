...
def analizar_sentimiento(texto: str) -> str:
    ...
    try:
        response = ollama.chat(
            model=MODELO_OLLAMA,
            messages=[
                {'role': 'system', 'content': 'Eres un sistema de clasificación estricto. Solo puedes responder con una única palabra. Nada de explicaciones.'},
    ...
    for archivo in archivos_csv:
        ...
        try:
            ...
            if COLUMNA_TEXTO not in df.columns:
                print(f"Advertencia: La columna '{COLUMNA_TEXTO}' no se encuentra en {archivo}. Se omitirá.")
                continue
            ...
            df['sentimiento_ollama'] = df[COLUMNA_TEXTO].progress_apply(lambda x: analizar_sentimiento(str(x)))
