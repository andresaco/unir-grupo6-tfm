...
@asset(
    group_name="training",
    description="Entrena la red neuronal LSTM y registra los artefactos y métricas en MLflow.",
)
def lstm_model_training(
    ...
    prepara secuencias temporales para LSTM, entrena una red LSTM usando PyTorch, y la registra en MLflow.
    """
    ...
    validate_df(df, EngineeredFeaturesRow, stage="lstm_model_training (read features)")
    ...
    context.log.info("Iniciando el entrenamiento del modelo LSTM con PyTorch...")

    with mlflow.start_run(run_name="LSTM_NeuralNetwork") as run:
        ...
        model_info = mlflow.pytorch.log_model(
            ...
            registered_model_name=f"{config.name}_LSTM_Model",
        )
        ...
        try:
            context.log.info("Calculando explicabilidad SHAP para el modelo LSTM...")
            ...
            plot_path = "lstm_shap_summary.png"
            ...
            context.log.warning(
                f"No se pudo calcular la explicabilidad SHAP para LSTM: {e}"
            )
